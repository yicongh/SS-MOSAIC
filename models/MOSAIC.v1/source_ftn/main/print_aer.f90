...
!     WRITE OUTPUT:
!     ===========================================================================
      j_TOTMASS = (SUM(jk_PARMOLE_SOM,2) + j_GASMOLE_SOM)*1e6/NA*j_MW*1e6

      OPEN(UNIT=789,FILE='outputs/out.jTOTMASS',STATUS='UNKNOWN',ACCESS='APPEND')
      WRITE(789,'(14(E15.6))') time_dum, j_TOTMASS
      CLOSE(789)
...
!     ===========================================================================
      ...
      OLGMASS = &
      OLGMASS + jk_OLGMOLE_SOM(j,k)/(1e-9*NA*1e-6)*1.d-3*MW_AER_MAC(icn3_a+j-1)

      END DO
      END DO
      
      OPEN(UNIT=747,FILE='./outputs/out.OLGMASS',STATUS='unknown',ACCESS='append')
...
!     ===========================================================================
      ...
      WRITE(747,'(F15.4,5E15.5)') time_dum,SUM(jk_PARMOLE_SOM),SUM(jk_PARMOLE_OXY), &
                                  (SUM(jk_PARMOLE_OXY)+SUM(jk_OLGMOLE_OXY))/(SUM(jk_PARMOLE_SOM)+SUM(jk_OLGMOLE_SOM))/VOC_CNO, &
                                  SUM(j_GASMOLE_OXY)/SUM(j_GASMOLE_SOM)/VOC_CNO
      CLOSE(747)
      

      OPEN(UNIT=747,FILE='./outputs/out.jO2C',STATUS='unknown',ACCESS='append')
      WRITE(747,'(F15.4,13E15.5)') time_dum, SUM(jk_PARMOLE_OXY,2)/SUM(jk_PARMOLE_SOM,2)/VOC_CNO
      CLOSE(747)
